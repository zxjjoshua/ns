...
UdpClient::Send (void)
{
  ...
  SeqTsHeader seqTs;
  seqTs.SetSeq (m_sent);
  uint8_t buffer[10]="client su";
  //Ptr<Packet> p = Create <Packet> (buffer,10); // 8+4 : the size of the seqTs header
  Packet p(&buffer[0],10);
  p.AddHeader (seqTs);
//std::cout <<p->ToString()<<std::endl;
  ...
  if ((m_socket->Send (&p)) >= 0)
	...
	NS_LOG_INFO("TraceDelay TX content is"<<m_size);
    }
  else
    {
