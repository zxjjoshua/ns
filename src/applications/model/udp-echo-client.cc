...
UdpEchoClient::SetFill (std::string fill)
{
  ...
  NS_LOG_INFO("UdpEchoClient::SetFill");
  printf("printf SetFill\n");
...
void
UdpEchoClient::SendTo (char* message)
{
  NS_LOG_FUNCTION (this);

  NS_ASSERT (m_sendEvent.IsExpired ());
  m_dataSize = std::strlen(message) + 1;
  memcpy (m_data, message, m_dataSize);
  Ptr<Packet> p;
  if (m_dataSize)
    {
      //
      ...
      // to agree with m_dataSize
      //
      NS_ASSERT_MSG (m_dataSize == m_size, "UdpEchoClient::SendTo(): m_size and m_dataSize inconsistent");
      NS_ASSERT_MSG (m_data, "UdpEchoClient::SendTo(): m_dataSize but no m_data");
      p = Create<Packet> (m_data, m_dataSize);
    }
  else
    {
